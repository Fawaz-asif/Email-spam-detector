...
const formatPercent = (value?: number) => (typeof value === "number" ? `${(value * 100).toFixed(1)}%` : "N/A");
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast.error("API Connection Error", {
        description: `Make sure Flask API is running on ${API_URL}`,
      });
    } finally {
      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl">
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex items-center justify-center gap-3 mt-4 flex-wrap">
            ...
            <Badge variant="outline" className="px-3 py-1">F1 Score: {formatPercent(modelInfo?.f1_score)}</Badge>
            <Badge variant="outline" className="px-3 py-1">Accuracy: {formatPercent(modelInfo?.accuracy)}</Badge>
            <Badge variant="outline" className="px-3 py-1">
              {modelInfo?.feature_count ? `${modelInfo.feature_count} Features` : "Features: N/A"}
            </Badge>
          </div>
        </div>
              ...
              <Card className={`shadow-xl border-2 ${
                ...
                <CardContent className="space-y-4">
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      Prediction made by trained <strong>{modelInfo?.model_name ?? "spam detection"}</strong> model
                      with {formatPercent(modelInfo?.accuracy)} accuracy and {formatPercent(modelInfo?.f1_score)} F1 score.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{modelInfo?.model_name ?? "N/A"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">F1 Score:</span>
                    <span className="font-semibold text-success">{formatPercent(modelInfo?.f1_score)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Accuracy:</span>
                    <span className="font-semibold text-success">{formatPercent(modelInfo?.accuracy)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Precision:</span>
                    <span className="font-semibold">{formatPercent(modelInfo?.precision)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Recall:</span>
                    <span className="font-semibold">{formatPercent(modelInfo?.recall)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Features:</span>
                    <span className="font-semibold">{modelInfo?.feature_count ? `${modelInfo.feature_count} TF-IDF` : "N/A"}</span>
                  </div>
                </div>
